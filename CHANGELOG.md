...
- **Ordner-Variante für gesperrte/gehärtete Rechner (neu):** Dieselbe Version 10.10 gibt es jetzt zusätzlich als **Ordner-ZIP** (`WellDone_10.10_Ordner.zip`, über die Download-Seite bzw. die Releases-Seite). Sie entpackt beim Start **nicht** in den Temp-Ordner und ist **nicht UPX-gepackt** – dadurch startet sie auch auf abgesicherten Windows-Rechnern, die die normale Einzeldatei blockieren (Ausführung aus `%TEMP%` per Richtlinie gesperrt) oder deren Virenscanner sie fälschlich beanstandet. Einfach entpacken und die `WellDone_10.10.exe` **im Ordner** starten (keine Installation, keine Administratorrechte; eine `START-ANLEITUNG.txt` liegt in der ZIP). Der Programminhalt ist identisch zur Einzeldatei.
